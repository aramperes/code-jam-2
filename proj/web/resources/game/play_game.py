            ...
            if self.user_data["username"] == game["turn"]:
                ...
                if valid:
                    ...
                    if self.user_data["username"] == game["defender_username"]:
                        ...
                        self.db.run(self.db.query("games").get(data["id"]).update("turn":game["challenger_username"]))

                        if update == "attacker":
                            self.db.run(self.db.query("games").get(data["id"]).update("defender_stats":updated_stats))
                        else:
                            self.db.run(self.db.query("games").get(data["id"]).update("challenger_stats":updated_stats))

                        if special and update == "attacker":

                            health_update = updated_stats
                            health_update["health"] -= 1
                            self.db.run(self.db.query("games").get(data["id"]).update("defender_stats":health_update))
                        ...
                        if special and update == "attacker":
                            ...
                            health_update = updated_stats
                            health_update["health"] -= 1
                            self.db.run(self.db.query("games").get(data["id"]).update("challenger_stats":health_update))
